...
import Brat.Checker.Monad (Checking, CheckingSig(..), captureOuterLocals, defineEnd, err,  kindArgRows, isSkolem, mkYield, throwLeft, typeErr)
...
simpleCheck my ty tm = case (my, ty) of
  (Braty, VApp (VPar e) _) -> do
    isHope <- case e of
      ExEnd _ -> pure False
      InEnd e -> req AskHopes <&> M.member e
    if isHope then
      case tm of
...
vecLayers my ty = req AskHopes >>= \h -> case (my, ty) of
  (_, (VApp (VPar e) _)) | InEnd ie <- e, M.member ie h ->
    mkYield "getThunks" (S.singleton e) >> vecLayers my ty
  (my, (TVec ty (VNum n))) -> do
    src <- buildNatVal n
    first ((src, n):) <$> vecLayers my ty
  (Braty,(VFun Braty cty)) -> pure ([], cty)
  (Kerny,(VFun Kerny cty)) -> pure ([], cty)
  (my, ty) -> typeErr $ "Expected a " ++ showMode my ++ "function or vector of functions, got " ++ show ty
